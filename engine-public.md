...
3. [Create a service ID API key](/docs/iam?topic=iam-serviceidapikeys) or select an existing one.
...
You can view and access your classic infrastructure credentials from the **Access (IAM)** section of the console only if you are a classic infrastructure user. If you do not have classic infrastructure access, the VPN username and classic infrastructure API key fields do not display on the page. For more information, see [Managing classic infrastructure access](/docs/iam?topic=iam-mngclassicinfra).
{: important}
...
3. In the API keys section, [create a classic infrastructure API key](/docs/iam?topic=iam-classic_keys) or find your existing key.
...
5. Assign your user permissions to manage DNS in the account.
   ...
   For more information about managing classic infrastructure access, see [Classic infrastructure permissions](/docs/iam?topic=iam-mngclassicinfra#how-classic-infra-permissions-work).
