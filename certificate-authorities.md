...
| Prerequisites |
| :------------ |
| Before you connect Let's Encrypt, be sure that you:  /n  /n - Obtain the private key that's associated with your Automatic Certificate Management Environment (ACME) account.  /n  /n The ACME protocol makes it possible to automatically obtain browser trusted certificates without human intervention. Before you can request Let's Encrypt certificates through {{site.data.keyword.secrets-manager_short}}, you must have an ACME account registered. If you already have a working ACME account, you'll need the private key that was generated when the account was initially created.  /n  /n - Optional: Create an ACME account.  /n  /n If you don't have an existing ACME client or account, you can create one by using the [ACME account creation tool](https://github.com/ibm-cloud-security/acme-account-creation-tool){: external}.{: note} |
