...
Deploy {{site.data.keyword.secrets-manager_short}} instances using the [Secrets Manager module](https://registry.terraform.io/modules/terraform-ibm-modules/secrets-manager/ibm/latest){: external} for consistent, version-controlled infrastructure. See [Terraform IBM Modules](/docs/ibm-cloud-provider-for-terraform?topic=ibm-cloud-provider-for-terraform-about-tim) for details.
{: tip}
...
Event notifications are now also logged and forwarded.
:   {{site.data.keyword.secrets-manager_short}} is now forwarding also either an `INFO` or `ERROR` level log for any event notification that is sent.
...
:   You can now use {{site.data.keyword.secrets-manager_short}} to set up a private certificate authority (CA) that you can use to issue SSL/TLS certificates to your internal applications. By configuring root and intermediate Certificate Authorities for your instance, you can establish a valid chain of trust for your certificates.
