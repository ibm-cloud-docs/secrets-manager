...
| [IAM credentials](/docs/secrets-manager?topic=secrets-manager-iam-credentials) | `iam_credentials` | Dynamic | A dynamically generated service ID and API key that can be used to access an {{site.data.keyword.cloud_notm}} service that requires IAM authentication. |<imported-cert>
| [Certificates](/docs/secrets-manager?topic=secrets-manager-certificates) | `imported_cert` | Static | An SSL or TLS certificate that can be used to establish commnication privacy. |</imported-cert>
