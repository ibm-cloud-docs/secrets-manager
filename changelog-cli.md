...
- Added support for using the CLI plug-in to interact with {{site.data.keyword.secrets-manager_short}} service instance over a private network connection.
- Fixed an issue with [config-update](/docs/secrets-manager?topic=secrets-manager-cli-plugin-secrets-manager-cli#secrets-manager-cli-config-update-command)
