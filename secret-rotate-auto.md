...
When you rotate a secret in {{site.data.keyword.secrets-manager_short}}, you create a new version of its value. By scheduling automatic rotation of your secrets at regular intervals, you can reduce the likelihood of compromise and ensure that your secrets never expire.

Automatic rotation is available only for secrets that are generated by {{site.data.keyword.secrets-manager_short}}. If the secret was imported initially, you must provide new secret data to rotate it. For more information, see [Manually rotating secrets](/docs/secrets-manager?topic=secrets-manager-manual-rotate-secrets).
{: note}
